...
void updateLogicA(char[][NUM_COLS], int[][NUM_COLS], int[][NUM_COLS],
			      int[][NUM_COLS], bool[][NUM_COLS], bool[][NUM_COLS], int[][NUM_COLS]);
			      
char check4gameWinner(int[][NUM_COLS], bool[][NUM_COLS], bool&);

void usersTurn(char[][NUM_COLS], char[][NUM_COLS], bool[][NUM_COLS]);
	...
	while((tournOn==1)||(rematch==1))
	{
		...
		if((gameNum==1)||(lastWinner == 't')) //If it is the first game of a new Tournament (or there was a tie for the first game(s))
		{
			...
			if(flipResult==1)
			{
				...
				while(gameOn==1)
				{
					usersTurn(board, key, spotsTakenArray);
					updateLogicA(board, LA1, LA2, LA3, spotsTakenArray, goodMoveArray, LA4);
					displayBoard(board, key, compWinTot, userWinTot);
...
void updateLogicA(char boxes[][NUM_COLS], int logic1[][NUM_COLS], int logic2[][NUM_COLS],
			      int logic3[][NUM_COLS], bool spotTaken[][NUM_COLS], bool goodMove[][NUM_COLS], int logic4[][NUM_COLS])
{
...
void usersTurn(char square[][NUM_COLS], char legend[][NUM_COLS], bool spotsTake[][NUM_COLS])
{
	int choice = 0;
	int reset = 1;
